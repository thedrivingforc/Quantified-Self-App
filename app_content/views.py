    ...
    try:
        if request.method == 'POST':
            ...
            if tracker and current_user_id == tracker.user_id:
                flash('The tracker "' + name +
                      '" is already added by you.', category='error')
...
def add_log(record_id):
    ...
    try:
        if request.method == 'POST':
            ...
            db.session.commit()
            flash('New Log Added For ' + this_tracker.name +
...
def view_tracker(record_id):
    ...
    try:
        import sqlite3
        con = sqlite3.connect('app_content/database.db')
        ...
        plt.ylabel('Values')
        plt.tight_layout()
        plt.savefig('app_content/static/Images/graph.png')
        # plt.show()

        gon = sqlite3.connect('app_content/database.db')
        g = gon.cursor()
...
def edit_log(record_id):
    ...
    try:
        if request.method == 'POST':
            ...
            db.session.commit()
            flash(this_tracker.name +
                  ' Log Updated Successfully.', category='success')
